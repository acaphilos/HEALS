...
<div class="container h-100">
  <div class="hero-content text-white">
    <div class="row">
      <div class="title">
        <h2>All Schedule Appointment List</h2>
      </div>
    </div>
  </div>
</div>
    ...
    <tbody>
      <?php
        if (!empty($all_appointments)) {
            foreach ($all_appointments as $row) {
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form id="form-<?php echo $row->id; ?>" method="post" action="<?php echo base_url('ScheduledAppointmentList/updateDoctorRemark/' . $row->id); ?>" style="display:none;">
                    ...
                    if ($row->status == 'Scheduled') {
                        ...
                        echo '<form method="post" action="' . base_url('ScheduledAppointment/viewUpdateSchedule/' . $row->id) . '" onsubmit="return confirmApprove(' . $row->id . ')">
                              <button type="submit" value="Approve" class="m-2 btn btn-success"><h6 class ="text-white">Complete</h6></i></button>
                              </form>';

                        echo '<form method="post" action="' . base_url('ScheduledAppointment/viewUpdateSchedule/' . $row->id) . '" onsubmit="return confirmDecline(' . $row->id . ')">
                          <button type="submit" value="Decline" class="m-2 btn btn-secondary"><h6 class ="text-white">Change Schedule</h6></i></button>
                          </form>';

                        echo '<form method="post" action="' . base_url('ScheduledAppointment/endSchedule/' . $row->id) . '" onsubmit="return confirmDelete(' . $row->id . ')">
                      <button type="submit" value="Delete" class="m-2 btn btn-danger"><h6 class ="text-white">End</h6></button>
                      </form>';
                    } elseif ($row->status == 'Pending' ) {
                        
                        echo '<form method="post" action="' . base_url('ScheduledAppointment/viewUpdateSchedule/' . $row->id) . '" onsubmit="return confirmComplete(' . $row->id . ')">
                              <button type="submit" value="Completed" class="m-2 btn btn-secondary"><h6 class ="text-white">Change Schedule</h6></i></button>
                              </form>';
                      ...
                      <button type="submit" value="Delete" class="m-2 btn btn-danger"><h6 class ="text-white">End</h6></button>
                        ...
                        echo '<form method="post" action="' . base_url('ScheduledAppointment/deleteSchedule/' . $row->id) . '" onsubmit="return confirmDelete(' . $row->id . ')">
            ...
            ?>
            <tr>
                <td colspan="8">You have no Scheduled appointments</td>
            </tr>
            <?php
        }
        ?>
