...
<section class="timetable">
  ...
  <div class="container d-flex flex-column justify-content-center align-items-center">
    <h2 class="mb-4">All Schedules Appointment</h2>
    <a href="<?=base_url()?>ScheduledAppointmentList">
    <button type="button" class="btn btn-info"><i class='bx bx-history nav_icon'></i> View All Schedules Appointment</button>
    </a>
  </div>

  <div class="container d-flex flex-column justify-content-center align-items-center">
    <h2 class="mt-5">Create new schedule appointment</h2>
        ...
        if (!empty($all_users)) {
            foreach ($all_users as $row) {
                ...
                <tr>
                    ...
                    echo '<form method="post" action="' . base_url('ScheduledAppointment/viewAddSchedule/' . $row->uId) . '" onsubmit="return confirmUpdate(' . $row->uId . ')">
                          <button type="submit" value="Update" class="m-2 btn btn-success"><h6 class ="text-white">Add</h6></i></button>
                          </form>';

                    echo '<form method="post" action="' . base_url('ScheduledAppointmentList/viewUserScheduleAdmin/' . $row->uId) . '" onsubmit="return confirmUpdate(' . $row->uId . ')">
